    ...
    if call.method == "getPlatformVersion" { // Verifica si se está llamando el método "getPlatformVersion"
      ...
      if connectedPeripheral?.state == CBPeripheralState.connected {
        ...
        if let characteristic = targetCharacteristic {
            guard let listbytes = self.bytes else {
                result(false)
                return
            }
            ...
            // result(true) fires via didWriteValueFor on first chunk ack
        ...
        if let characteristic = targetCharacteristic {
            if self.stringprint.count > 0 {
                    ...
                    // ESC @ initializes the printer — must come BEFORE text, not after
                    let initBytes: [UInt8] = [0x1b, 0x40]
                    // ESC d 3 — feed 3 lines so the printed text exits the cutter zone
                    let feedBytes: [UInt8] = [0x1b, 0x64, 0x03]
                    ...
                    connectedPeripheral?.writeValue(Data(initBytes), for: characteristic, type: writeType)
                    // 2. Set font size
                    connectedPeripheral?.writeValue(Data(sizeBytes[size]), for: characteristic, type: writeType)
                    // 3. Text + LF — LF is required to flush the line buffer and trigger printing
                    let data = Data((texto + "\n").utf8)
                    connectedPeripheral?.writeValue(data, for: characteristic, type: writeType)
                    // 4. Feed paper so printed text is visible
                    connectedPeripheral?.writeValue(Data(feedBytes), for: characteristic, type: writeType)
                    stringprint = ""
