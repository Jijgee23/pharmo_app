...
class BasketProvider extends ChangeNotifier {
  ...
  Future<dynamic> checkQTYs() async {
    try {
      ...
      if (_shoppingCarts.isNotEmpty) {
        ...
        final response = await http.patch(Uri.parse('${dotenv.env['SERVER_URL']}check_qty/'),
        ...
        if (response.statusCode == 200) {
          ...
          for (String key in res.keys) {
            ...
            dynamic cart = shoppingCarts.firstWhere((s) => s['product_itemname_id'] != null ? s['product_itemname_id'].toString() == key : s['product_id'].toString() == key);
            if (res[key] == null) {
  ...
  Future<dynamic> addBasket({int? product_id, int? itemname_id, required int qty}) async {
    try {
      ...
      if (check['errorType'] == 1) {
        String bearerToken = await getAccessToken();
        final response = await http.post(Uri.parse('${dotenv.env['SERVER_URL']}basket_item/'),
            headers: <String, String>{
              'Content-Type': 'application/json; charset=UTF-8',
              'Authorization': bearerToken,
            },
  ...
  Future<dynamic> clearBasket({required int basket_id}) async {
    try {
      ...
      String? token = prefs.getString("access_token");
      String bearerToken = "Bearer $token";
      final response = await http.post(Uri.parse('${dotenv.env['SERVER_URL']}clear_basket/'),
          headers: <String, String>{
            'Content-Type': 'application/json; charset=UTF-8',
            'Authorization': bearerToken,
          },
  ...
  Future<dynamic> checkPayment() async {
    try {
      String bearerToken = await getAccessToken();
      final resQR = await http.get(Uri.parse('${dotenv.env['SERVER_URL']}cp/'), headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
        'Authorization': bearerToken,
      });
      if (resQR.statusCode == 200) {
  ...
  Future<dynamic> removeBasketItem({required int basket_id, required int item_id}) async {
    try {
      String bearerToken = await getAccessToken();
      final resQR = await http.delete(Uri.parse('${dotenv.env['SERVER_URL']}basket_item/$item_id/'), headers: <String, String>{
        'Content-Type': 'application/json; charset=UTF-8',
        'Authorization': bearerToken,
      });
  ...
  Future<dynamic> changeBasketItem({required String type, required int item_id, required int qty}) async {
    try {
      ...
      final resQR = await http.patch(Uri.parse('${dotenv.env['SERVER_URL']}basket_item/$item_id/'),
          headers: <String, String>{
            'Content-Type': 'application/json; charset=UTF-8',
            'Authorization': bearerToken,
          },
